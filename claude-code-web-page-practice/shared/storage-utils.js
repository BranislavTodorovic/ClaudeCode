...
(function (root, factory) {
  ...
  function asset(v) { return v == null || (text(v,500) && /^assets\/[\w/-]+\.(svg|webp|png|jpg)$/.test(v)); }
  function photoSource(v) { return v == null || (object(v) && text(v.alt,300,true) && text(v.credit,200,true) && !!v.sourceUrl && url(v.sourceUrl) && text(v.license,80,true) && !!v.licenseUrl && url(v.licenseUrl)); }
  function destination(x) { return object(x) && id(x.id) && text(x.name, 100, true) && text(x.country, 100, true) && choices(x.categories,['city break','nature','beach','culture/history','food','wellness','adventure']) && ['low','medium','high'].includes(x.budget) && choices(x.seasons,['spring','summer','autumn','winter']) && choices(x.duration,['weekend','week','extended']) && choices(x.styles,['relaxed','balanced','active']) && ['warm','temperate','cool'].includes(x.climate) && ['Europe','North America','Asia-Pacific'].includes(x.region) && tags(x.tags) && text(x.summary, 1000, true) && text(x.details, 5000, true) && asset(x.image) && asset(x.cardImage) && asset(x.heroImage) && asset(x.fallbackImage) && optionalText(x.imageAlt,300) && photoSource(x.photoSource) && text(x.fallback, 100, true) && resources(x.links); }
    ...
    try {
      if (arrayKeys.indexOf(key) >= 0) {
        ...
        return a.every(function (x) {
          ...
          if (/personal-/.test(key)) return text(x.text,80,true) && (x.done == null || typeof x.done === 'boolean') && (x.frequency == null || ['','daily','weekly','monthly'].includes(x.frequency)) && (x.target == null || (key === 'orbit-personal-habits' && Number.isInteger(x.target) && x.target >= 1 && x.target <= 365)) && timestamps(x,false);

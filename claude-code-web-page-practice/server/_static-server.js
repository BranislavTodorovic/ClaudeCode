...
function createServer({root=path.resolve(__dirname,'..'),providerOptions={},missingAsset=''}={}){
 const api=require('./api').createApi({root,...providerOptions});
 ...
 return http.createServer((req,res)=>{
  ...
  try{
   ...
   const decoded=decodeURIComponent(url.pathname),parts=decoded.split(/[\\/]+/);
   if(missingAsset&&decoded==='/'+missingAsset)return fail(404);

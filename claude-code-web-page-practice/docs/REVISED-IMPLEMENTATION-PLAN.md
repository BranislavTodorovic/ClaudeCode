...
## Post-R0 scope amendment — credential-free default

After Gate 12 and R0, bundled/local catalogue data is the default production discovery path. Provider adapters remain supported architecture for **optional explicitly approved integrations**, but no TMDB/RAWG/IGDB/destination credential is required for core OneSpace or final release. Any historical wording below that sounds mandatory must be interpreted through this rule. The browser/provider security boundary remains mandatory whenever an optional provider is configured.

The redesign may also use properly licensed repository-local raster/vector scene/media assets in addition to CSS/SVG/procedural layers; the prior no-new-binary-assets constraint is no longer an architecture requirement.

## Canonical post-R0 architecture/execution boundary

R0 is a **classification/change-control gate only**. It freezes the baseline, resolves historical conflicts and records the architecture delta; it does not implement redesign UI. The redesign is then delivered in six controlled stages: R1 shared architecture/design/asset foundation, R2 Home fidelity proof, R3 main-world rollout, R4 Work->Projects nested pilot, R5 submodule-family rollout, and R6 cross-world integration/regression. Only after Gate R6 does execution continue into the R0-classified Phase 14–25 obligations.

Testing architecture must support deterministic time/date injection for time-sensitive UI, a canonical deterministic dataset, migration backup/rollback fixtures, isolated first-run/empty states, multi-tab collision tests, large-data stress, malformed preference/import-schema recovery, animation cancellation and listener/timer/RAF cleanup checks.

---

# 1. Product model

OneSpace is a local-first personal dashboard with eight **reference-led redesigned main worlds**:
...
Existing Shortcuts, Productivity, Notes and legacy Projects routing remain compatibility/functionality obligations and are classified at R0 as dedicated routes, subviews/portals or safe aliases.

Home is the only **general all-domain summary aggregator**. `Projects & Notes` is a deliberately constrained two-domain presentation/portal surface over canonical Work-project and Notes ownership; it does not become a second general aggregator and does not create parallel stores. Every owning domain remains authoritative for its records.
...
# 3. Local-first, credential-free default with optional remote enrichment

User data remains local.

Bundled/local discovery is the normal production path. Provider-backed discovery is optional enrichment for rich-content domains only when explicitly approved/configured.
...
Local/bundled discovery remains usable without provider functionality. If an explicitly approved optional provider is configured and becomes unavailable, only that enrichment displays an explicit degraded state; credential-free core discovery and saved content remain usable.

---

# 4. Optional provider domains
...
V2 supports optional provider-neutral global place/destination discovery when explicitly approved/configured; it is not required for the default product.

Bundled/local destination data is the default. Optional provider selection is implementation work only if explicitly approved, because licensing, imagery and practical API access must then be evaluated.
...
# 5. Optional provider adapter contract
...
# 6. Optional provider search state machine
...
# 7. Local + optional-provider search composition
...
# 25. Reference-led main-world scenes

After R0, exactly eight redesigned main worlds use the approved reference-led scene system:
...
These remain one coherent premium warm-interior visual family. Shortcuts, Productivity, Notes and legacy Projects retain the visual treatment assigned by R0 and are not automatically additional reference-led worlds. Base scenes work with repository-local assets and no provider art.
...
Final acceptance includes local/imported rich-domain records in export/import; if optional-provider-origin records exist, they are included as an additional conditional case.
...
- base scenes survive;
- local/bundled discovery remains available; any configured optional provider enrichment becomes explicitly unavailable/degraded.
